    ...
    public class FtiProgramService : IFtiProgramService
    {
            ...
            FtiProgramContentWithChildrenCreateDto dto)
        {
            ...
            try
            {
                ...
                var parentEntity = _mapper.MapToEntity(new FtiProgramContentCreateDto
                {
                    Title = dto.Title,
        ...
        public async Task<ServiceResult<FtiResourcePersonDto>> AddResourcePersonAsync(
            int contentId,
            FtiResourcePersonCreateDto dto)
        {
            var content = await _contentRepository.GetByIdAsync(contentId);

            if (content == null)
                return ServiceResult<FtiResourcePersonDto>.Failure(
            ...
            return ServiceResult<FtiResourcePersonDto>.Success(resultDto);
        }

        public async Task<ServiceResult<FtiResourcePersonDto>> UpdateResourcePersonAsync(
            int personId,
            FtiResourcePersonUpdateDto dto)
        {
            ...
            var content = await _contentRepository.GetByIdAsync(person.FtiProgramContentAndResourcesId ?? 0);
            var program = await _programRepository.GetByIdAsync(content?.FtiProgramDetailsId ?? 0);
        ...
        public async Task<ServiceResult<List<FtiResourcePersonDto>>> GetResourcePersonsByContentIdAsync(int contentId)
        {
            var content = await _contentRepository.GetByIdAsync(contentId);

            if (content == null)
                return ServiceResult<List<FtiResourcePersonDto>>.Failure(
                    "Content not found",
                    ServiceErrorStatus.NOTFOUND);
            ...
            return ServiceResult<List<FtiResourcePersonDto>>.Success(dtos);
        }
        ...
        public async Task<ServiceResult<FtiTopicsCoveredDto>> AddTopicAsync(
            int contentId,
            FtiTopicsCoveredCreateDto dto)
        {
            var content = await _contentRepository.GetByIdAsync(contentId);

            if (content == null)
                return ServiceResult<FtiTopicsCoveredDto>.Failure(
                    "Program content not found",
                    ServiceErrorStatus.NOTFOUND);
            ...
            return ServiceResult<FtiTopicsCoveredDto>.Success(resultDto);
        }

        public async Task<ServiceResult<FtiTopicsCoveredDto>> UpdateTopicAsync(
            int topicId,
            FtiTopicsCoveredUpdateDto dto)
        {
            ...
            var content = await _contentRepository.GetByIdAsync(topic.FtiProgramContentAndResourcesId ?? 0);
            var program = await _programRepository.GetByIdAsync(content?.FtiProgramDetailsId ?? 0);
        ...
        public async Task<ServiceResult<List<FtiTopicsCoveredDto>>> GetTopicsByContentIdAsync(int contentId)
        {
            var content = await _contentRepository.GetByIdAsync(contentId);

            if (content == null)
                return ServiceResult<List<FtiTopicsCoveredDto>>.Failure(
                    "Content not found",
                    ServiceErrorStatus.NOTFOUND);
            ...
            return ServiceResult<List<FtiTopicsCoveredDto>>.Success(dtos);
        }
        ...
        public async Task<ServiceResult<FtiTeachingAidsDto>> AddTeachingAidAsync(
            int contentId,
            FtiTeachingAidsCreateDto dto)
        {
            var content = await _contentRepository.GetByIdAsync(contentId);

            if (content == null)
                return ServiceResult<FtiTeachingAidsDto>.Failure(
                    "Program content not found",
                    ServiceErrorStatus.NOTFOUND);
            ...
            return ServiceResult<FtiTeachingAidsDto>.Success(resultDto);
        }

        public async Task<ServiceResult<FtiTeachingAidsDto>> UpdateTeachingAidAsync(
            int aidId,
            FtiTeachingAidsUpdateDto dto)
        {
            ...
            var content = await _contentRepository.GetByIdAsync(aid.FtiProgramContentAndResourcesId ?? 0);
            var program = await _programRepository.GetByIdAsync(content?.FtiProgramDetailsId ?? 0);
        ...
        public async Task<ServiceResult<List<FtiTeachingAidsDto>>> GetTeachingAidsByContentIdAsync(int contentId)
        {
            var content = await _contentRepository.GetByIdAsync(contentId);

            if (content == null)
                return ServiceResult<List<FtiTeachingAidsDto>>.Failure(
                    "Content not found",
                    ServiceErrorStatus.NOTFOUND);
            ...
            return ServiceResult<List<FtiTeachingAidsDto>>.Success(dtos);
        }
