// NaepProgramDetailsRepository.cs
...
{
    public class NaepProgramDetailsRepository : INaepProgramDetailsRepository
    {
        private readonly TdmsDbContext _context;

        public NaepProgramDetailsRepository(TdmsDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<NaepProgramDetails?> GetWithDetailsAsync(int id)
        {
            return await _context.NaepProgramDetails
        ...
        public async Task<NaepProgramDetails> CreateAsync(NaepProgramDetails entity)
        {
            _context.NaepProgramDetails.Add(entity);
            await _context.SaveChangesAsync();
            return await GetWithDetailsAsync(entity.Id) ?? entity;
        }

        public async Task<NaepProgramDetails> UpdateAsync(NaepProgramDetails entity)
        {
            _context.NaepProgramDetails.Update(entity);
            await _context.SaveChangesAsync();
            return entity;
        }
        ...
        {
            var entity = await _context.NaepProgramDetails.FindAsync(id);
            if (entity != null)
            {
                _context.NaepProgramDetails.Remove(entity);
                await _context.SaveChangesAsync();
            }
        }

        public async Task<PaginatedResult<NaepProgramDetails>> GetPaginatedAsync(
        ...
        {
            var query = _context.NaepProgramDetails
                .Include(p => p.ProgramType)
                .Include(p => p.Category)
                .Include(p => p.Mode)
                .Include(p => p.Region)
            ...
            return new PaginatedResult<NaepProgramDetails>(items, totalItems, pageNumber, pageSize);
        }

        public async Task<PaginatedResult<NaepProgramDetails>> GetByStatusAsync(
            List<int> unitLocationIds,
            string status,
            int pageNumber = 1,
            int pageSize = 10)
