...
namespace Infrastructure.Services.DataTables.NAEP
{
    public class NaepProgramService : INaepProgramService
    {
        ...
        public async Task<ServiceResult<NaepResourcePersonDto>> AddResourcePersonAsync(
            int contentId,
            NaepResourcePersonCreateDto dto)
        {
            var content = await _contentRepository.GetByIdAsync(contentId);

            if (content == null)
                return ServiceResult<NaepResourcePersonDto>.Failure(
            ...
            return ServiceResult<NaepResourcePersonDto>.Success(resultDto);
        }

        public async Task<ServiceResult<NaepResourcePersonDto>> UpdateResourcePersonAsync(
            int personId,
            NaepResourcePersonUpdateDto dto)
        {
            ...
            var content = await _contentRepository.GetByIdAsync(person.NaepProgramContentAndResourcesId ?? 0);
            var program = await _programRepository.GetByIdAsync(content?.NaepProgramDetailsId ?? 0);
        ...
        public async Task<ServiceResult<List<NaepResourcePersonDto>>> GetResourcePersonsByContentIdAsync(int contentId)
        {
            var content = await _contentRepository.GetByIdAsync(contentId);

            if (content == null)
                return ServiceResult<List<NaepResourcePersonDto>>.Failure(
                    "Content not found",
                    ServiceErrorStatus.NOTFOUND);
            ...
            return ServiceResult<List<NaepResourcePersonDto>>.Success(dtos);
        }
        ...
        public async Task<ServiceResult<NaepTopicsCoveredDto>> AddTopicAsync(
            int contentId,
            NaepTopicsCoveredCreateDto dto)
        {
            var content = await _contentRepository.GetByIdAsync(contentId);

            if (content == null)
                return ServiceResult<NaepTopicsCoveredDto>.Failure(
                    "Program content not found",
                    ServiceErrorStatus.NOTFOUND);
            ...
            return ServiceResult<NaepTopicsCoveredDto>.Success(resultDto);
        }

        public async Task<ServiceResult<NaepTopicsCoveredDto>> UpdateTopicAsync(
            int topicId,
            NaepTopicsCoveredUpdateDto dto)
        {
            ...
            var content = await _contentRepository.GetByIdAsync(topic.NaepProgramContentAndResourcesId ?? 0);
            var program = await _programRepository.GetByIdAsync(content?.NaepProgramDetailsId ?? 0);
        ...
        public async Task<ServiceResult<List<NaepTopicsCoveredDto>>> GetTopicsByContentIdAsync(int contentId)
        {
            var content = await _contentRepository.GetByIdAsync(contentId);

            if (content == null)
                return ServiceResult<List<NaepTopicsCoveredDto>>.Failure(
                    "Content not found",
                    ServiceErrorStatus.NOTFOUND);
            ...
            return ServiceResult<List<NaepTopicsCoveredDto>>.Success(dtos);
        }
        ...
        public async Task<ServiceResult<NaepTeachingAidsDto>> AddTeachingAidAsync(
            int contentId,
            NaepTeachingAidsCreateDto dto)
        {
            var content = await _contentRepository.GetByIdAsync(contentId);

            if (content == null)
                return ServiceResult<NaepTeachingAidsDto>.Failure(
                    "Program content not found",
                    ServiceErrorStatus.NOTFOUND);
            ...
            return ServiceResult<NaepTeachingAidsDto>.Success(resultDto);
        }

        public async Task<ServiceResult<NaepTeachingAidsDto>> UpdateTeachingAidAsync(
            int aidId,
            NaepTeachingAidsUpdateDto dto)
        {
            ...
            var content = await _contentRepository.GetByIdAsync(aid.NaepProgramContentAndResourcesId ?? 0);
            var program = await _programRepository.GetByIdAsync(content?.NaepProgramDetailsId ?? 0);
        ...
        public async Task<ServiceResult<List<NaepTeachingAidsDto>>> GetTeachingAidsByContentIdAsync(int contentId)
        {
            var content = await _contentRepository.GetByIdAsync(contentId);

            if (content == null)
                return ServiceResult<List<NaepTeachingAidsDto>>.Failure(
                    "Content not found",
                    ServiceErrorStatus.NOTFOUND);
            ...
            return ServiceResult<List<NaepTeachingAidsDto>>.Success(dtos);
        }
            ...
            int pageSize = 10)
        {
            ...
            if (trainerId.HasValue && trainerId.Value > 0)
            {
                ...
                {
                    return new PaginatedResult<NaepProgramDetailsDto>(
                        new List<NaepProgramDetailsDto>(),
                        0,
                        pageNumber,
                        pageSize);
                }
