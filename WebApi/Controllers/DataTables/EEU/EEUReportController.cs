...
namespace WebApi.Controllers.DataTables.EEU
{
    ...
    [ApiController]
    [Authorize]
    public class EEUReportController : ControllerBase
    {
        private readonly TdmsDbContext _context;

        public EEUReportController(TdmsDbContext context)
        {
            _context = context;
        }

        // =======================================================
        // COMBINED ENDPOINT: EEU Monthly Report with Hierarchy Support
        // =======================================================
        ...
        public async Task<IActionResult> GetMonthlyReport(int unitId, int year, int month, int? unitLocationId = null)
        {
            ...
            if (!trainerIds.Any())
                ...
                {
                    UnitId = unitId,
                    UnitName = "Extension Education Unit",
            ...
            var frontLineDemonstrations = await _context.EeuProgramDetails
                .Include(p => p.Type)
            ...
            var fldData = new
            {
                ...
                Farmers = await _context.EeuParticipantDemographics
                    .Where(d => frontLineDemonstrations.Select(f => f.Id).Contains(d.EeuProgramDetailsId ?? 0))
                    .SumAsync(d => (d.Male_SC ?? 0) + (d.Male_ST ?? 0) + (d.Male_OBC ?? 0) + (d.Male_GEN ?? 0) +
                                   (d.Female_SC ?? 0) + (d.Female_ST ?? 0) + (d.Female_OBC ?? 0) + (d.Female_GEN ?? 0))
            };

            // ---------- (B) FARM TRIALS ----------
            var farmTrials = await _context.EeuProgramDetails
                .Include(p => p.Category)
                .Where(p => trainerIds.Contains(p.CreatedById ?? 0) &&
                            p.StartDate >= startDateOnly &&
                            p.StartDate < endDateOnly &&
            ...
            var ftData = new
            {
                ...
                Farmers = await _context.EeuParticipantDemographics
                    .Where(d => farmTrials.Select(f => f.Id).Contains(d.EeuProgramDetailsId ?? 0))
                    .SumAsync(d => (d.Male_SC ?? 0) + (d.Male_ST ?? 0) + (d.Male_OBC ?? 0) + (d.Male_GEN ?? 0) +
                                   (d.Female_SC ?? 0) + (d.Female_ST ?? 0) + (d.Female_OBC ?? 0) + (d.Female_GEN ?? 0))
            };
        ...
        private async Task<object> GetExtensionActivitiesData(List<int> trainerIds, int year, int month)
        {
            ...
            var allPrograms = await _context.EeuProgramDetails
                .Include(p => p.Category)
                .Include(p => p.Type)
            ...
            async Task<int> GetParticipantCount(List<int> programIds)
            {
                ...
                return await _context.EeuParticipantDemographics
                    .Where(d => programIds.Contains(d.EeuProgramDetailsId ?? 0))
                    .SumAsync(d => (d.Male_SC ?? 0) + (d.Male_ST ?? 0) + (d.Male_OBC ?? 0) + (d.Male_GEN ?? 0) +
                                   (d.Female_SC ?? 0) + (d.Female_ST ?? 0) + (d.Female_OBC ?? 0) + (d.Female_GEN ?? 0));
            }
            ...
            var telephoneConsultancy = await _context.EeuAdvisoryServices
            ...
            var faceToFaceConsultancy = await _context.EeuAdvisoryServices
                .Include(a => a.ProgramDetails)
                .Where(a => trainerIds.Contains(a.ProgramDetails.CreatedById ?? 0) &&
                            a.ProgramDetails.StartDate >= startDateOnly &&
                            a.ProgramDetails.StartDate < endDateOnly)
            ...
            var smsCount = await _context.EeuAdvisoryServices
                .Include(a => a.ProgramDetails)
                .Where(a => trainerIds.Contains(a.ProgramDetails.CreatedById ?? 0) &&
                            a.ProgramDetails.StartDate >= startDateOnly &&
                            a.ProgramDetails.StartDate < endDateOnly)
            ...
            // 19. WhatsApp SMS
            var whatsappCount = await _context.EeuAdvisoryServices
                .Include(a => a.ProgramDetails)
                .Where(a => trainerIds.Contains(a.ProgramDetails.CreatedById ?? 0) &&
                            a.ProgramDetails.StartDate >= startDateOnly &&
                            a.ProgramDetails.StartDate < endDateOnly)
